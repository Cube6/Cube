...
namespace Cube.Infrastructure.Redis
{
	public interface IRedisInstance
	{
		...
		public Task<bool> HashAddAsync<K, F, V>(K key, F field, V value, int expire = 0);
		public Task<bool> HashRemoveAsync<K, F>(K key, F field);
		public Task<V> HashGetAsync<K, F, V>(K key, F field);

		public Task<long> ListAddAsync<K, M>(K key, M member, int expire = 0);
		...
		public Task<bool> SetAddAsync<K, V>(K key, V value, int expire = 0);
